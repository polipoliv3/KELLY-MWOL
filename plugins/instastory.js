...
Kelly.addCommand({pattern: 'story ?(.*)', fromMe: false, desc: "Downloads STORY from instagaram" , dontAddCommandList: true }, async (message, match) => {
  ...
  const {data} = await axios(`https://xteam.xyz/dl/igs?nama=${match[1]}&APIKEY=ab9942f95c09ca89`)
    const { status, result } = data
     if(!status) return await message.sendMessage('api error')
         let Kelly =  await axios.get(`${result.url}`)
         const msg = `${result.type}`

    reply = await message.client.sendMessage(message.jid,UPLOAD_ING , MessageType.text, { quoted: message.data });
    
    if (type === 'image') return await message.sendMessage(Buffer.from(Kelly.data), MessageType.image, { caption: "*ᴍᴀᴅᴇ ʙʏ ᴋᴇʟʟʏ*", quoted: message.data })
    
    if (type === 'video') return await message.sendMessage(Buffer.from(Kelly.data), MessageType.video, { caption: "*ᴍᴀᴅᴇ ʙʏ ᴋᴇʟʟʏ*", quoted: message.data })

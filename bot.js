...
const {Message, StringSession, Image, Video} = require('./Kelly/');
        ...
        if (config.WORKTYPE == 'public') {
            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                ...
                if (conn.user.jid === '@s.whatsapp.net') {
                    ...
                    await conn.sendMessage(conn.user.jid, '*ᴋᴇʟʟʏ ᴍᴡᴏʟ sᴛᴀʀᴛᴇᴅ*', MessageType.text);
                }
            }
            else {
...
*/
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~𝙺𝙴𝙻𝙻𝚈 𝙼𝚆𝙾𝙻~*' +
                                    '\n\n*🧞‍♂️ ' + error + '*\n\n```Report🥺 errors\njoin with \nhttps://chat.whatsapp.com/EXjFwp6nhYm36uxUyLvaHM ``` ' 
                                    , MessageType.text);
                            }
                        }
                    }

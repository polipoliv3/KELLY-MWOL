...
re-coded or modified by afnanplk-kelly
to bring new ai
...
if (conf.LANG == 'EN') vtalk_dsc = 'Starts to kelly voice chat.', reply_eva = '*Reply to Any Voice Message!*'
...
Kelly.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('kelly') && conf.TALKING_KELLY !== 'true') {        
        var unique_ident = message.client.user.jid.split('@')[0]      
        let acc = os.userInfo().homedir.split('Whats')[1].split('Duplicated/')[0] == 'Kelly' ? '7d57838203msh0c5cf65c90a7231p13b461jsn77c8cfa55871' : '7d57838203msh0c582jak19865261js1229n77c8cfa55871'
        let aitalk_mode = message.message.includes('{normal}') ? 'raw' : 'waifu'
        var finm = message.message.replace('kelly', '').replace(' ', '')   
        var ainame = os.userInfo().homedir.split('Whats')[1].split('Duplicated/')[0]
        if (ainame !== 'Kelly') return;
...
Kelly.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
        if (conf.TALKING_KELLY == 'true' && ((!message.jid.includes('-')) || (message.jid.includes('-') && 
...
Kelly.addCommand({ pattern: 'vtalk$', desc: vtalk_dsc,dontAddCommandList: true, fromMe: wk }, (async (message, match) => {
...
var succ_off = ''
if (conf.LANG == 'TR') {
    fulleva_dsc = 'Tam fonksiyonel kelly özelliklerini aktif eder. Hesabınızı bir chatbota dönüştürün!'
    already_on = 'kelly yapay zekası halihazırda tüm fonksiyonları etkin.'
    already_off = 'kelly yapay zekası halihazırda yarı fonksiyonel çalışıyor.'
    succ_on = 'kelly, Tam Fonksiyonel Olarak Açıldı! Lütfen Biraz Bekleyin! ✅'
    succ_off = 'kelly, Yarı Fonksiyonel Olarak Ayarlandı! Lütfen Biraz Bekleyin! ☑️'
}
if (conf.LANG == 'EN') {
    fulleva_dsc = 'Activates full functional kelly features. Turn your account into a ai chatbot!'
    already_on = 'kelly artificial intelligence is already fully functional.'
    already_off = 'kelly artificial intelligence is currently running semi-functional.'
    succ_on = 'kelly Opened Fully Functionally! Please wait a bit! ✅'
    succ_off = 'kelly Set to Semi-Functional! Please wait a bit! ☑️'
}
if (conf.LANG == 'ML') {
    fulleva_dsc = 'പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായ kelly സവിശേഷതകൾ സജീവമാക്കുന്നു. നിങ്ങളുടെ അക്കൗണ്ട് ഒരു ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'
    already_on = 'kelly കൃത്രിമബുദ്ധി ഇതിനകം പൂർണ്ണമായി പ്രവർത്തിക്കുന്നു.'
    already_off = 'kelly AI നിലവിൽ സെമി-ഫംഗ്ഷണൽ ആണ്.'
    succ_on = 'kelly പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായി തുറന്നു! കുറച്ച് കാത്തിരിക്കൂ! ✅'
    succ_off = 'സെമി-ഫങ്ഷണൽ ആയി kelly സജ്ജമാക്കുക! കുറച്ച് കാത്തിരിക്കൂ! ☑️'
}

Kelly.addCommand({ pattern: 'kelly ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.kelly on / off' }, (async (message, match) => {
    var kelly_status = `${conf.TALKING_KELLY}`
    if (match[1] == 'on') {
        if (kelly_status == 'true') {
